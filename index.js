...
function sortByPricing(product1, product2) {
  return product2.price - product1.price;
}

app.get('/products/sort/price-high-to-low', (req, res) => {
  let sortedProducts = products.slice();
  sortedProducts.sort(sortByPricing);
  res.json({ products: sortedProducts });
});

// Endpoint 3

function sortByLowToHighPricing(product1, product2) {
  return product1.price - product2.price;
}

app.get('/products/sort/price-low-to-high', (req, res) => {
  let sortedProducts = products.slice();
  sortedProducts.sort(sortByLowToHighPricing);
  res.json({ products: sortedProducts });
});
...
app.get('/products/filter/ram', (req, res) => {
  ...
  let sortedProducts;
  sortedProducts = products.filter((products) => filterByRam(products, ram));
  res.json({ products: sortedProducts });
});
...
app.get('/products/filter/rom', (req, res) => {
  ...
  let sortedProducts;
  sortedProducts = products.filter((products) => filterByRom(products, rom));
  res.json({ products: sortedProducts });
});
...
app.get('/products/filter/os', (req, res) => {
  let os = req.query.os;
  let sortedProducts;
  sortedProducts = products.filter((products) => filterByOs(products, os));
  res.json({ products: sortedProducts });
});
